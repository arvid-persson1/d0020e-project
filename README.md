...
Run a package using `cargo run --release`. Use the `--package` flag and provide the package path to run the specified package. The `--release` flag indicates release (optimized) mode, which means long build times. Omit it to build in debug mode. See [The Cargo Book](https://doc.rust-lang.org/cargo/) for more commands.

## Getting started

**This is primarily a library**. See [documentation](#Documentation) to get started with integrating.

To try a quick demo, first start up some sources:
```sh
cargo run --package bookery &
cargo run --package rest-api-axum &
```
...
Or interactively using our frontend:
```sh
cargo run --package api-broker
# Open api-broker/index.html in your browser.
```

## Documentation

See [setup](#Setup) for instructions on how to install required tools.
...
Get started with the `Broker` struct as well as the `Source` and `Sink` traits.
...
- Presence of documentation (using `cargo doc`), including detection of broken links, missing "Errors" or "Panics" sections where applicable, and verifying that code snippets compile.
